...
import { ExternalConfigInterface, View } from "../../interface/io/view.ts";
import { AIO, DoNothing, pDeno, pfs, plog } from "../../resource/types/purify.ts";
import { Context, newContext, u32 } from "../../resource/types/types.ts";
	...
	async (s: State<Meta>): Promise<AIO<State<Meta>>> => {
		...
		return (
			asker
				...
				.noAwait((finalState) => cfg.externalConfig.saveTo.map(f => pDeno.writeTextFile(f, save(finalState.db))).getOrElse(() => DoNothing))
				.and((finalState) =>
